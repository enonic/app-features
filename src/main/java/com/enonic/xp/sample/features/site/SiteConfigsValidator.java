            ...
            if ( siteDescriptor != null )
            {
                ...
                catch ( final InputTypeValidationException e )
                {
                    validationErrorsBuilder.add( ValidationError.dataError(
                            ValidationErrorCode.from( ApplicationKey.from( "com.enonic.app.features" ), "my-app.validation.siteConfigFieldInvalid" ),
                            e.getPropertyPath() )
                        .i18n( "my-app.cms.validation.siteConfigFieldInvalid" )
